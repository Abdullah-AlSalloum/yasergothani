...
const FALLBACK_SHEET_GID = 756041902;
...
export async function POST(request: Request) {
  try {
    ...
    const privateKey = getEnv("GOOGLE_PRIVATE_KEY")
      ...
      .replace(/^['\"]|['\"]$/g, "")
      .replace(/\\n/g, "\n");
    const spreadsheetId = FALLBACK_SHEET_ID;

    if (!spreadsheetId) throw new Error("FALLBACK_SHEET_ID is not configured.");
    ...
    const targetGid = Number(process.env.GOOGLE_CASE_STUDY_SHEET_GID) || FALLBACK_SHEET_GID;
    const sheetTitle =
