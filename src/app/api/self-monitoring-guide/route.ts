...
const SELF_MONITORING_FALLBACK_SHEET_ID = "16N4_Y4mQJ-nyYr1kFRObxrvnkOIxI8z-YsIAHyP_agU";
const SELF_MONITORING_FALLBACK_SHEET_GID = 104857961;
...
export async function POST(request: Request) {
  try {
    ...
    const privateKey = getEnv("GOOGLE_PRIVATE_KEY")
      ...
      .replace(/^['\"]|['\"]$/g, "")
      .replace(/\\n/g, "\n");
    const spreadsheetId = SELF_MONITORING_FALLBACK_SHEET_ID;
    ...
    const targetGid = Number(process.env.GOOGLE_SELF_MONITORING_SHEET_GID) || SELF_MONITORING_FALLBACK_SHEET_GID;
    const firstSheetTitle =
      spreadsheet.data.sheets?.find((s) => s.properties?.sheetId === targetGid)?.properties?.title ??
