...
const TARGET_SHEET_GID = "1279095454";
...
export async function POST(request: Request) {
  try {
    ...
    if (!targetSheetTitle) {
      return Response.json({ error: `تعذر العثور على تبويب الموارد (GID: ${TARGET_SHEET_GID}).` }, { status: 500 });
    }

    const headerRange = `${targetSheetTitle}!A1:F1`;
