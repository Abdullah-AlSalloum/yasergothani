...
const TARGET_ADS_SHEET_GID = "965869592";
...
export async function POST(request: Request) {
  try {
    ...
    const targetSheetTitle = resolveTargetSheetTitle(
      ...
      undefined,
      TARGET_ADS_SHEET_GID,
    );

    if (!targetSheetTitle) {
      return Response.json({ error: `تعذر العثور على تبويب إدارة الحملات الإعلانية (GID: ${TARGET_ADS_SHEET_GID}).` }, { status: 500 });
    }

    const headerRange = `${targetSheetTitle}!A1:P1`;
