    ...
    <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-8">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8">
        {resources.map((res, idx) => (
          <div key={idx} className="bg-white border border-[#113c56] rounded-2xl shadow-md px-4 py-8 flex flex-col items-center text-center relative transition-all duration-300 hover:shadow-xl hover:-translate-y-2 ">
            <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-[#e6eef5] border border-[#113c56] rounded-full p-3 shadow" style={{ color: '#113c56' }}>
              <InsertDriveFileOutlinedIcon fontSize="large" style={{ color: '#113c56' }} />
            </span>
            <h3 className="text-xl font-bold text-[#113c56] mt-6 mb-2">{res.title}</h3>
            <p className="text-gray-500 mb-6 text-base">{res.subtitle}</p>
            <a
              href={res.file}
              download
              className="border font-bold py-2 px-8 rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
              style={{ borderColor: '#113c56', color: '#113c56' }}
              onMouseOver={e => { e.currentTarget.style.background = '#e6eef5'; e.currentTarget.style.color = '#113c56'; }}
              onMouseOut={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#113c56'; }}
            >
              <InsertDriveFileOutlinedIcon fontSize="small" style={{ color: '#113c56' }} />
              تحميل الآن
            </a>
          </div>
        ))}
