...
const categories = [
  ...
  "المتاجر الإلكترونية",
  "الناشرين والمؤلفين",
  "المشاريع الرقمية والناشئة",
  "الخبراء والمدربين وأصحاب العلامات الشخصية",
];

export default function TargetAudienceSection() {
  return (
    <section className="w-full py-16 px-4" style={{ background: 'var(--background)' }}>
      <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-8">
        ...
        <div className="text-lg md:text-xl font-semibold text-primary mb-4" style={{ color: 'var(--foreground)', fontFamily: 'Tajawal, Arial' }}>
          نعمل مع :
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full mb-8">
          {categories.map((cat, i) => (
            <div
              key={i}
              className="relative bg-white/10 backdrop-blur-xl rounded-2xl p-6 flex flex-col items-center text-center shadow-lg border border-primary-light transition-transform duration-300 hover:scale-105 hover:shadow-2xl group"
              style={{ minHeight: 120 }}
            >
              ...
              <div className="relative z-10 text-lg md:text-xl font-bold text-primary" style={{ fontFamily: 'Tajawal, Arial' }}>{cat}</div>
            </div>
          ))}
        </div>
